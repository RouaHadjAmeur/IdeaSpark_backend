...
export class SloganAiService {
  ...
  async generateSlogans(dto: GenerateSlogansDto): Promise<GenerateSlogansResponseDto> {
    ...
    if (dto.copywritingPrompt) {
      ...
      // S'assurer que le format JSON est demandé avec le schéma exact
...
Génère entre 5 et 10 slogans variés. Ne mets aucun texte avant ou après le JSON.`;
    ...
    try {
      ...
      let parsedResponse;
      try {
        // Nettoyer la réponse au cas où il y aurait du texte avant ou après le JSON
      ...
      // Relax requirement: take what we have, up to 10
      if (slogans.length === 0) {
        throw new Error('No slogans were generated by the AI');
      }
...
Genera 10 eslóganes VARIADOS y CREATIVOS.`,
  ...
  private parseSlogans(response: any): SloganDto[] {
    ...
    // Cas 2: La réponse a une clé 'slogans' qui est un tableau
    else if (response.slogans && Array.isArray(response.slogans)) {
      rawSlogans = response.slogans;
    }
    // Cas 3: La réponse a une clé 'data' ou 'results'
    else if (response.data && Array.isArray(response.data)) {
      rawSlogans = response.data;
    }
    else if (response.results && Array.isArray(response.results)) {
      rawSlogans = response.results;
    }
    // Cas 4: C'est un objet unique qui ressemble à un slogan
    else if (response.slogan) {
      rawSlogans = [response];
    }
    else {
      console.error('[AI Service] Invalid response structure. Expected array or {slogans: [...]}, got:', JSON.stringify(response, null, 2));
      throw new Error('Invalid response format from Gemini');
    }

    return rawSlogans.map((item: any) => ({
      slogan: item.slogan || item.text || item.content || '',
      explanation: item.explanation || item.description || item.reason || '',
      memorabilityScore: Math.min(100, Math.max(0, item.memorabilityScore || item.score || 70)),
      category: item.category || item.type || 'Descriptif',
    })).filter(s => s.slogan.length > 0);
  }
